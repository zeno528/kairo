...
kairo_register_group network "🌐 网络与代理"
...
kairo_register_module firewall system "防火墙管理🛡" "status install open_port close_port allow_ip block_ip delete_rule enable disable allow_listeners"
kairo_register_module port-proc system "端口/任务管理器" "listen_ports find_by_port find_by_name list_memory list_cpu kill_process"
...
kairo_register_module docker system "Docker 管理✨" "list_containers start stop restart remove logs exec stats images compose cleanup status install upgrade uninstall reset overview"
