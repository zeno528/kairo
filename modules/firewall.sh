...
do_status() {
    echo ""
    if [ -z "$FW" ]; then
        echo "  错误: 未检测到防火墙工具 (ufw/firewalld/iptables)"
        return
    fi
    echo "  防火墙: $FW"
    case "$FW" in
        ufw)
            echo "  状态: $(ufw status | head -1 | sed 's/Status: //')"
            echo ""
...
do_open_port() {
    ...
    case "$FW" in
        ufw)
            sudo ufw allow "$port/$proto" && echo "  已开放 $port/$proto"
            ;;
        firewalld)
        ...
        iptables)
            sudo iptables -A INPUT -p "$proto" --dport "$port" -j ACCEPT \
                && echo "  已开放 $port/$proto (当前会话，重启后失效)"
            ;;
    esac
}
...
do_close_port() {
    ...
    case "$FW" in
        ufw)
            sudo ufw delete allow "$port/$proto" && echo "  已关闭 $port/$proto"
            ;;
        firewalld)
        ...
        iptables)
            sudo iptables -A INPUT -p "$proto" --dport "$port" -j DROP \
                && echo "  已关闭 $port/$proto (当前会话，重启后失效)"
            ;;
    esac
}
...
do_enable() {
    ...
    case "$FW" in
        ufw)
            echo "  警告: 确保已放行 SSH 端口 (22)，否则可能无法远程连接"
            read -p "  确认开启? [y/N]: " confirm
            [ "$confirm" != "y" ] && [ "$confirm" != "Y" ] && echo "  已取消" && return
            sudo ufw enable && echo "  防火墙已开启"
            ;;
        firewalld)
            sudo systemctl enable --now firewalld && echo "  防火墙已开启"
            ;;
        iptables)
            echo "  iptables 无全局开关，需手动管理规则"
            ;;
    esac
}
...
do_disable() {
    ...
    case "$FW" in
        ufw)
            sudo ufw disable && echo "  防火墙已关闭"
            ;;
        firewalld)
            sudo systemctl stop firewalld && sudo systemctl disable firewalld && echo "  防火墙已关闭"
            ;;
        iptables)
            echo "  iptables 无全局开关，需手动清空规则"
            ;;
    esac
}
